 ...
 * </table>
 */
public class CoreOutputText extends UIXOutput
{
  static public final FacesBean.Type TYPE = new FacesBean.Type(
    UIXOutput.TYPE);
  static public final PropertyKey TRUNCATE_AT_KEY =
    TYPE.registerKey("truncateAt", Integer.class, Integer.valueOf(0));
  static public final PropertyKey DESCRIPTION_KEY =
    TYPE.registerKey("description", String.class);
  static public final PropertyKey ESCAPE_KEY =
    TYPE.registerKey("escape", Boolean.class, Boolean.TRUE);
  ...
  static public final String COMPONENT_TYPE =
    "org.apache.myfaces.trinidad.CoreOutputText";

  /**
   * Construct an instance of the CoreOutputText.
   */
  public CoreOutputText()
  {
    super("org.apache.myfaces.trinidad.Text");
  }

  /**
   * Gets the length at which the text should automatically begin
   ...
   *
   * @return  the new truncateAt value
   */
  final public int getTruncateAt()
  {
    return ComponentUtils.resolveInteger(getProperty(TRUNCATE_AT_KEY), 0);
  }

  /**
   * Sets the length at which the text should automatically begin
   *               truncating.  When set to zero (the default), the string
   *               will never truncate.  Values from one to thirteen will
   *               be treated as thirteen - the outputText component will not
   *               truncate strings shorter than thirteen characters.
   * 
   * @param truncateAt  the new truncateAt value
   */
  final public void setTruncateAt(int truncateAt)
  {
    setProperty(TRUNCATE_AT_KEY, Integer.valueOf(truncateAt));
  }

  /**
   * Gets an additional description to be attached
  ...
  {
    return ComponentUtils.resolveString(getProperty(DESCRIPTION_KEY));
  }

  /**
   * Sets an additional description to be attached
   * to the text, for use in improving accessibility.  The description will
   * be available to screen readers and other accessibility agents, but will
   * not be otherwise visible.
   * 
   ...
   */
  final public boolean isEscape()
  {
    return ComponentUtils.resolveBoolean(getProperty(ESCAPE_KEY), true);
  }

  /**
   * Sets <html> an attribute controlling whether output will
   * be escaped for the current markup language or not.  This attribute
   * defaults to true, and should only be set to false if absolutely necessary.
   * When set to false, you may exposed to cross-site scripting attacks if
   * the value of this component is any way derived from values supplied
   ...
   */
  final public void setEscape(boolean escape)
  {
    setProperty(ESCAPE_KEY, escape ? Boolean.TRUE : Boolean.FALSE);
  }
